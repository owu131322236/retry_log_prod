@props(['post'])
    <div class="rounded-3xl bg-white shadow-lg h-fit w-full my-2 hover:border-primary/50 hover:scale-105 transition-all">
        <div class="flex w-full flex-row items-start justify-start gap-3 p-4" 
             ...
             <img
                    src="{{ $post->user->icon?->path ? asset($post->user->icon->path) : asset('storage/images/icons/default.jpg') }}"
                    alt="User Icon"
            ...
            <div class="flex h-full flex-1 flex-col items-start justify-start">
                <div class="flex w-full flex-row gap-3 items-start justify-start m-2">
                    <div class="post-short-profile flex items-center gap-1">
                        <p class="text-[#0d0d1c] text-sm font-bold leading-normal tracking-[0.015em]">{{ $post->user->name }}</p>
        ...
        <div class="flex flex-wrap gap-2 px-6 py-2">
            <div class="reactions-wrapper relative flex flex-col items-center">
                <div class="reactions-button flex justify-center items-center rounded-2xl border border-gray-300 py-2 px-3 hover:bg-white hover:border hover:border-blue-600">
                    ...
                    <div class="reactions-menu hidden absolute top-[40px] left-0 flex bg-white rounded-2xl border border-gray-300 p-2">
                        @foreach ($post->available_reactions as $reaction)
                        <form method="POST" action="{{ route('posts.reaction.toggle')}}" class="inline-block">
                            @csrf
                            <input type="hidden" name="target_type" value="{{ $post->getMorphClass() }}">
            ...
            <div class="reactions flex gap-2">
                @foreach ($post->reactionCounts as $count)
            ...
            <button class="replyFormButton flex items-center justify-center rounded-2xl border border-gray-300 gap-2 px-3 hover:border-blue-600"
                    ...
                    data-reply-user-icon="{{ $post->user->icon?->path ? asset($post->user->icon->path) : asset('storage/images/icons/default.jpg')}}"
    ...
    <!-- <script>
        function goToProfile(e){
            e.stopPropagation();
            window.location.href='{{ route('mypage',[$post->user]) }}';
        }
...
<!-- <script>
    document.querySelectorAll('.reactions-button').forEach(button => {
        button.addEventListener('click', function(e) {
            e.stopPropagation(); // 他のクリックイベントに影響しないように
            const reactionMenu = this.querySelector('.reactions-menu');
